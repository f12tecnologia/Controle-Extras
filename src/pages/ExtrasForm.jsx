...
const ExtrasForm = () => {
  ...
  const addExtra = () => {
    setExtras([...extras, { ...initialExtraState }]);
  };
  ...
  const fetchDropdownData = useCallback(async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Não foi possível carregar funcionários ou empresas."
      });
    }
  }, [toast, user]);
  ...
  const loadExtraForEdit = useCallback(async extraId => {
    ...
    setLoading(true);
    try {
      const data = await replitDb.getExtra(extraId);
      if (!data) {
        toast({
          variant: "destructive",
          title: "Erro ao carregar extra",
          description: "Este extra não existe."
        });
        navigate('/my-extras');
        return;
      }
  ...
  const handleSubmit = async e => {
    ...
    try {
      if (id) {
        ...
        await replitDb.updateExtra(id, payload);
      } else {
        for (const extra of extras) {
          ...
          await replitDb.createExtra(payload);
        }
      }
      toast({
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select required value={employeeId} onValueChange={setEmployeeId} disabled={!!id}>
                  ...
                  <SelectContent className="bg-slate-800 border-white/20">
                    {employees.map(emp => (
                      <SelectItem key={emp.id} value={emp.id.toString()} className="text-white hover:bg-white/10">
                        {emp.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
                ...
                <Select required value={companyId} onValueChange={setCompanyId} disabled={!!id}>
                  ...
                  <SelectContent className="bg-slate-800 border-white/20">
                    {companies.length > 0 ? companies.map(company => (
                      ...
                      <div className="text-sm text-gray-400 p-2">Nenhuma empresa cadastrada.</div>
                    )}
                  </SelectContent>
                </Select>
              </div>
                ...
                <CardHeader>
                  ...
                  {!id && (
                    <Button 
                      ...
                      className="absolute top-4 right-4 text-red-500 hover:bg-red-500/10 hover:text-red-400"
                    >
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="space-y-2 lg:col-span-3">
                      ...
                      <Input 
                        id={`data_evento-${index}`} 
                        type="date" 
                        value={extra.data_evento} 
                        onChange={e => handleExtraChange(index, 'data_evento', e.target.value)} 
                    ...
                    <div className="space-y-2">
                      ...
                      <Input 
                        id={`hora_entrada-${index}`} 
                        type="time" 
                        value={extra.hora_entrada} 
                        onChange={e => handleExtraChange(index, 'hora_entrada', e.target.value)} 
                        className="input-glow bg-white/10 border-white/20 text-white" 
                        required 
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input 
                        id={`hora_saida-${index}`} 
                        type="time" 
                        value={extra.hora_saida} 
                        onChange={e => handleExtraChange(index, 'hora_saida', e.target.value)} 
                        className="input-glow bg-white/10 border-white/20 text-white" 
                        required 
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input 
                        ...
                        className="input-glow bg-white/10 border-white/20 text-white placeholder:text-gray-400" 
                        placeholder="0.00" 
                        required 
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input 
                        id={`setor-${index}`} 
                        value={extra.setor} 
                        onChange={e => handleExtraChange(index, 'setor', e.target.value)} 
                        className="input-glow bg-white/10 border-white/20 text-white placeholder:text-gray-400" 
                    ...
                    <div className="space-y-2">
                      ...
                      <Input 
                        id={`vaga-${index}`} 
                        value={extra.vaga} 
                        onChange={e => handleExtraChange(index, 'vaga', e.target.value)} 
                        className="input-glow bg-white/10 border-white/20 text-white placeholder:text-gray-400" 
        ...
        {!id && (
          <Button 
            ...
            size="sm" 
            onClick={addExtra} 
            className="w-full border-dashed border-white/30 text-gray-300 hover:bg-white/10"
          >
